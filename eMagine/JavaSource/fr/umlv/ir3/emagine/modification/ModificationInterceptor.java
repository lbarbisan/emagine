...
public class ModificationInterceptor extends EmptyInterceptor {
    ...
    public boolean onFlushDirty(Object obj, Serializable id, Object[] newValues, Object[] oldValues,
    	...
    	//V�rifie que l'object est bien un objet persitent
    ...
    public void postFlush(Iterator arg0) throws CallbackException {
        ...
        try {
            for (Iterator itr = inserts.iterator(); itr.hasNext();) {
                Modification logRecord = (Modification) itr.next();
 ...
 * @param className the name of the class being logged.  Used as a reference in the Modification
          ...
          Class interfaces[] = fields[ii].getType().getInterfaces();
              ...
              if(event.equals(UPDATE)) {
                  ...
                  if (propertyNewState.equals(propertyPreUpdateState)) {
                      ...
                      Modification logRecord = new Modification();
                      ...
                      if(parentObject == null) {
                          logRecord.setEntity((BaseEntity) newObject);
                      } else {
                          logRecord.setEntity((BaseEntity) parentObject);
                      }
                      ...
                      modification.setEntityName(className);
                      modification.setEntityAttribute(fieldName); 
                      modification.setMessage(event);
                      modification.setUpdatedBy(this.getUserName());
                      modification.setUpdatedDate(new Date());
                      modification.setOldValue("");

                      if (returnValue != null) {
                          modification.setNewValue(returnValue.toString());
                      } else
                          modification.setNewValue("");
 

                      if(parentObject == null) {
                          modification.setEntity((BaseEntity) newObject);
                      } else {
                          modification.setEntity((BaseEntity) parentObject);
                      }

                      inserts.add(modification);
