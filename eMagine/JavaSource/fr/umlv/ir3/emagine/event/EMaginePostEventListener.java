...
public class EMaginePostEventListener implements PostInsertEventListener,
	...
	public void onPostInsert(PostInsertEvent event) {
		if (event.getEntity() instanceof EventableEntity) {
			...
			try {
				eMagineEvent.setConnectedUser(SessionManager.getInstance()
	...
	private void logChanges(PostUpdateEvent postUpdateEvent)
		...
		if (postUpdateEvent.getEntity() instanceof EventableEntity) {

			// Iterate through all the fields in the object
			for (int index = 0; index < types.length; index++) {
				if (!"Collection".equals(types[index].getReturnedClass()
						.getSimpleName())) {
					...
					event.setNewValue(states[index] == null ? ""
							: states[index].toString());
					event.setOldValue(oldStates[index] == null ? ""
							: oldStates[index].toString());
					event.setProperty(fieldName);
					event.setTitle("Modification paramètres");
					if (!(states[index] == null && oldStates[index] == null)) {
						event.setDescription("Modification du paramètre '"
								+ fieldName + "' de '" + event.getOldValue()
								+ "' à '" + event.getNewValue() + "'");

						if (!event.getNewValue().equals(event.getOldValue())) {
							((EventableEntity) postUpdateEvent.getEntity())
									.getEvents().add(event);
							event.setSource((BaseEntity) postUpdateEvent
									.getEntity());
						}
					}
				}
